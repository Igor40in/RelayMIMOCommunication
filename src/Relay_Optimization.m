                ...
                for k=1:K
                    ...
                    for i=1:nRelayStation+1
                        ...
                        Beta(k,1) = Beta(k,1)*(lamda(k,i)*X(k,i)./(lamda(k,i)*X(k,i)+1));
                                ...
                                X(1:K_opt(iSNIR,iRealiz), iRelay ) = P0/(K_opt(iSNIR,iRealiz)*1.0)*ones(K,1);
%                 X(1:K, iRelay ) = P(1:K,iRelay)*0.8;
                %%only mimo
                %                 X(1:K1, iRelay ) = P;
                [metric,Parameters] = NMDS_RelayOptimization(X,iRelay,Beta,Pmin(1:K ),Pmax(1:K),Pdelta(1:K),lamda(1:K,iRelay),P0,optBudget);
        ...
        P_opt = zeros(K,nRelayStation+1);
        for iRelay = 1:nRelayStation+1
            if iRelay==1
                P_opt(:,iRelay) = X(:,iRelay);
            else
                P_opt(:,iRelay) = X(1:K,iRelay)./(lamda(1:K,iRelay-1).*X(1:K,iRelay-1)+1);
            end
        end
        K2 = K;
%         sum(1./(P.*lamda(1:K1)+1))
...
%         end
        ...
        for iRelay = 1:nRelayStation+1
            if iRelay==1
                FF{1} = V(:,1:K,iRelay)*sqrt(diag(P_opt(1:K,iRelay)) );
                x(:,1) =FF{1}*S(:,1) ;
            else
                FF{iRelay} = V(:,1:K,iRelay)*sqrt(diag(P_opt(1:K,iRelay)))*U(:,1:K,iRelay-1)';
        ...
        Hcommon2 = U(:,1:K,end)*diag(prod(sqrt(P_opt.*lamda(1:K,:)),2));
        %         Hcommon2 = Hcommon;
        ...
        1;
        yest = Hcommon2*symMod(1:K)+noiseCommmon;
        ...
        symStd_opt(cntSym_opt+1:cntSym_opt+K) = (S(:,end)-S(:,1));
