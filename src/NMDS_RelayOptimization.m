function [metric, Parameters] = NMDS_RelayOptimization(parAll_In,indRelay,Beta,parMin,parMax,parDelta,lamda,P0,optBudget)
parIn = parAll_In(:,indRelay);
nRelay = size(parAll_In,2);
nParam = size(parIn,1);
alpha = 1;
beta = 0.5;
gamma = 2;
...
metricStep = metricEvaluation(Param_real, lamda,Beta,P0);
metric(1) = metricStep;
ObFun(1) = metric;
...
%     'Reflection',Iteration
    ...
    Steps(Iteration) = Steps(Iteration)+1;
    metricRef = metricEvaluation(paramReal, lamda,Beta,P0);
...
%         strcat('Iteration=',num2str(Iteration),'_Reflection')
    ...
    if metricRef<=bestObFun
        ...
        Steps(Iteration) = Steps(Iteration)+1;
        metricExp = metricEvaluation(paramReal, lamda,Beta,P0);
...
%         strcat('Iteration=',num2str(Iteration),'Outside contraction')
        ...
        Steps(Iteration) = Steps(Iteration)+1;
        metricConOut =  metricEvaluation(paramReal, lamda,Beta,P0);
...
%     strcat('Iteration=',num2str(Iteration),'Inside contraction')
        ...
        Steps(Iteration) = Steps(Iteration)+1;
        metricConIns = metricEvaluation(paramReal, lamda,Beta,P0);
...
%     strcat('Iteration=',num2str(Iteration),'Shrink')
    ...
    for iVertx = 1:nParam+1
        ...
        Steps(Iteration)=Steps(Iteration)+1;
        metricShr = metricEvaluation(paramReal, lamda,Beta,P0);
...
function metric = metricEvaluation(parIn,lamda,Beta,P0)
p = sum(parIn);
% if p>P0
% %     'More P0'
% %     parIn = parIn./p*P0;
...
coeff2 = 2000000;
if l>0
    penaltyAdd = penaltyAdd+coeff1*P0/1*sum( abs( repmat(P0,length(indEdgeMore),1)-parIn(indEdgeMore) ) );
    penaltyAdd = penaltyAdd+coeff1*P0/1*sum( abs(parIn(indEdgeLow)) );
    penaltyAdd = penaltyAdd+coeff2*(p-P0);
elseif p>P0
    penaltyAdd = coeff2*(p-P0);
else
    penaltyAdd = 0;
end

metric = sum( 1-Beta.*lamda().*parIn./(1+lamda.*parIn) )+penaltyAdd;
metric2 = sum( 1./(parIn.*lamda+1) )+penaltyAdd;
if abs(metric-metric2)>0.001
    1;
end
